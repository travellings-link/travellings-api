...
router.get('/:id', async (req, res) => {
    if (!isInt(req.params.id) || !req.params.id) {
      return res.status(200).json({ success: false, msg: "有坏蛋，我不说是谁 ╭(╯^╰)╮~" });
    }
...
router.post('/add', async (req, res) => {
  ...
  if (!name || !link) {
    return res.status(200).json({ success: false, msg: "至少应该告诉我名称（name）和链接（link）吧 ヽ(‘⌒´メ)ノ" });
  }
  ...
  try {
    ...
    res.status(200).json({ success: false, msg: "出错了呜呜呜~ 请检查控制台输出喵~" });
  }
});
...
router.post('/edit', async (req, res) => {
    ...
    if (!id) {
      return res.status(200).json({ success: false, msg: "至少应该告诉我编号（id）吧 ヽ(‘⌒´メ)ノ" });
    }
    ...
    try {
      ...
      if (web) {
        ...
        res.status(200).json({ success: false, msg: "没找到这个站点 -_-#" });
      }
    } catch (error) {
        console.log(chalk.red(`[${global.time()}] [ERROR]`, error));
        res.status(200).json({ success: false, msg: "出错了呜呜呜~ 请检查控制台输出喵~" });
